	...
	float p_miss)
{
	...
	if (rowCount == 0 || colCount == 0)
	{
		throw std::invalid_argument("Maatrix size is zero Please try again");
	}
    ...
    for(size_t i=0; i<rowCount; ++i)
    {
		...
		{
			throw std::invalid_argument("Check Belief first row size");
		}
		if (gridRow.size() != colCount)
		{
			throw std::invalid_argument("Check Grid row first row size");
		}
	...
	float blurring)
{
	...
	if (rowCount == 0 || colCount == 0)
	{
		throw std::invalid_argument("Length of the matrix is zero try again");
	}
