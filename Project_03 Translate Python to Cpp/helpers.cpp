...
{
    float sum_ofgrid = 0.f;

    for(size_t i=0; i<grid.size(); ++i)
    {
        const vector<float> &cell = grid[i];
    ...
    const float mult_const = 1.f/sum_ofgrid;
    ...
    for(size_t i=0; i<grid.size(); ++i)
    {
        ...
        for(size_t j=0; j<cell.size(); ++j)
        {
            newRow.push_back(cell[j]*mult_const);
        }	// of cols
...
vector<vector<float> > blur(vector<vector<float> > grid, float blurring) {
	...
	{
		throw std::invalid_argument("Input Matrix containing data");
	}
    ...
    for(size_t i=0; i<rowCount; ++i)
    {
		...
		{
			throw std::invalid_argument("Check row lengths");
		}
